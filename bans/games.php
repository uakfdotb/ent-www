...
if ($user->data['user_id'] == ANONYMOUS || !isadmin($user->data['user_id'])) {
	...
	<br />Map: <input type="text" name="map" />
	...
	if(isset($_REQUEST['map'])) $extra_get .= "&map=" . $_REQUEST['map'];
	?>
	...
	//add map where clause (works with username)
	if(isset($_REQUEST['map']) && $_REQUEST['map'] != '') {
		//we have to escape map for the MySQL LIKE construct; we use = here to escape
		$map = likeEscape($_REQUEST['map'], '=');

		$where_add = " games.map LIKE ? ESCAPE '='";
		$parameters[] = "%$map%";
